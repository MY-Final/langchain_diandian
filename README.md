...
PostgreSQL 当前用于初始化会话记忆相关表结构，后续可接持久化记忆。
...
- `POSTGRES_DSN` 支持本地或远程 PostgreSQL，填写后优先使用。
- 如果不填 `POSTGRES_DSN`，则使用拆分字段组装连接参数。
- `POSTGRES_ENABLED=true` 时，启动 `chat_app` 或 `onebot_gateway` 会自动初始化短期记忆和长期记忆相关表。
- 也可以单独执行：`python -m chat_app.postgres`
...
`ONEBOT_OPERATOR_USER_IDS` 支持用英文逗号分隔多个用户 ID；这些用户在私聊中可以启用高风险技能，例如删好友、改资料。
...
- 当前已支持模型通过显式 tool 调用触发群管理技能中的结构化命令，例如禁言、设管理员、踢人、改名片、改头衔
...
当前项目已经新增 `chat_app/skills/`，用于按场景组织 agent 能力，而不是把所有底层 tool 全量暴露给模型。
...
- `docs/skill_extension_guide.md`
- `docs/skill_development_manual.md`
...
- 短期记忆：最近对话 + 滚动摘要
- 长期记忆：跨会话保留的重要事实、偏好、约束

相关文档：

- `docs/long_term_memory_design.md`
